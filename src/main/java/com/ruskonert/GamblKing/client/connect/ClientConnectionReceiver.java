...
/**
 * ClientConnectionReceiver 는 서버에 받은 패킷을 바탕으로 하여 작업을 처리하는 클래스입니다.
 * 이 클래스는 수신기능만 하며 이 클래스를 통한 발신 기능은 지원하지 않습니다.
 */
public class ClientConnectionReceiver
{
    private static GameServerConnection gameServerConnection;
    public static void setGameServerConnection(GameServerConnection c) { gameServerConnection = c;}
    public static GameServerConnection getGameServerConnection() { return gameServerConnection;}
    ...
    static void setPlayerEntityFramework(PlayerEntityFramework framework) { playerEntityFramework = framework; }
    ...
    private Task<Void> playerThread = new Task<Void>() {
        @Override
